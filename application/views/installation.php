...
<div class="col-md-8">
 ...
 if($last_sl=='/'){
	...
	$sq_url=$sq_url."/";  
		...
		$con = mysqli_connect($_POST['hostname'], $_POST['sq_dbusername'], $_POST['sq_dbpassword']);
...
$groupbymode=0;
		...
		<tr   <?php if (!is_writable('sample/')) { ?>style="background:#FF9999;"<?php } ?> ><th class='thead'>Writable Permission</th><td> sample/ </td><td><?php if (!is_writable('sample/')) { echo "Not writable";}else{ echo "Writable"; }  ?></td></tr>
...
?>
		...
		<div class="alert alert-danger" style="font-size:13px;"> Do you have any issue to fulfill minimum requirement?  Contact us at <a href="http://savsoftquiz.org">www.SavsoftQuiz.org</a> for professional installation service.</div>
		<hr><br>
		<form class="form-signin" method="post" action="<?php echo $sq_url.'index.php/Install/config_sq';?>" onSubmit="return agreetos();">
			...
			<div class="form-group">	 
					...
					<input type="text"   name="sq_base_url" class="form-control" value="<?php echo str_replace('index.php/Install/index/ph/','',str_replace('//index.php/Install/index/ph/','/',$sq_url));?>" placeholder="<?php echo $this->lang->line('hostname');?>" required autofocus >
			</div>
			<div class="form-group">	 
					<label for="inputEmail"  ><?php echo $this->lang->line('hostname');?></label> 
					<input type="text"   name="sq_hostname" class="form-control" value="<?php if(isset($_POST['hostname'])){ echo $_POST['hostname']; }?>" placeholder="<?php echo $this->lang->line('hostname');?>" required autofocus >
			</div>
			<div class="form-group">	 
					<label for="inputEmail" class="sr-only"><?php echo $this->lang->line('db_name');?></label> 
					<input type="text"  value="<?php if(isset($_POST['sq_dbname'])){ echo $_POST['sq_dbname']; }?>" name="sq_dbname" class="form-control" placeholder="<?php echo $this->lang->line('db_name');?>" required   >
			</div>
			<div class="form-group">	 
					<label for="inputEmail" class="sr-only"><?php echo $this->lang->line('db_username');?></label> 
					<input type="text"  value="<?php if(isset($_POST['sq_dbusername'])){ echo $_POST['sq_dbusername']; }?>" name="sq_dbusername" class="form-control" placeholder="<?php echo $this->lang->line('db_username');?>" required   >
			</div>
			<div class="form-group">	 
					<label for="inputEmail" class="sr-only"><?php echo $this->lang->line('db_password');?></label> 
					<input type="password" value="<?php if(isset($_POST['sq_dbpassword'])){ echo $_POST['sq_dbpassword']; }?>"  name="sq_dbpassword" class="form-control" placeholder="<?php echo $this->lang->line('db_password');?>"     >
			</div>
 			<div class="form-group">	  
					<input type="checkbox" name="tos" id="tos"> <span style="font-size:11px;">You must agree to our <a href="http://savsoftquiz.org/tos.php" target="savsoftquiz" >Terms & Conditions</a> before installation</span><br>
